...
$ curl -X GET http://$(oc get route istio-ingressgateway -n east-istio-system --template='{{.spec.host}}')/hello/env
-----
...
. *Getting Load Balancers from AWS clusters
+
-----
$ oc config use-context east-istio && export WEST_LB=$(oc get svc ingress-west -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' -n east-istio-system)
echo $WEST_LB

` Update this value to ossm_simple_federation/west/mesh-fed/servicemeshpeers.yml  `

$ oc config use-context west-istio && export EAST_LB=$(oc get svc ingress-east -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' -n west-istio-system)
echo $EAST_LB

` Update this value to ossm_simple_federation/east/mesh-fed/west/servicemeshpeers.yml  `
-----
...
$ for i in {1..1000}
  do
    curl -m 10 -X  GET http://$(oc get route istio-ingressgateway -n east-istio-system --template='{{.spec.host}}')/hello/env;
    echo ""
    sleep 2;
  done
-----
